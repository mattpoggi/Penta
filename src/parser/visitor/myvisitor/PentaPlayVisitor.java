...
/**
 * Visitor for music playing.
 * It gives no semantic check.
 * Callable only after semantic check by PentaParseVisitor if it gives positive result.
 * While parsing, it will play music only during the essay phase (sounds declared in strophes will only be played when 
 * strophes occurs in essay!)
 * @author Matteo Poggi
 */
public class PentaPlayVisitor implements IVoidVisitor {
	 ...
	 */
	public PentaPlayVisitor(JTextArea console) throws MidiUnavailableException {
	...
	public void visit(Scope n) {
		n.f0.accept(this);
		n.f1.accept(this);
		consolePrint("Playing "+n.f1.tokenImage+"...");
		n.f2.accept(this);
		n.f3.accept(this);
		n.f4.accept(this);
		n.f5.accept(this);
		consolePrint("\n"+n.f1.tokenImage+" played successfully!");
	}
	...
	public void visit(VelocityDeclaration n) {
		...
		n.f1.accept(this);
		n.f2.accept(this);
		this.player.setVelocity(Integer.parseInt(n.f2.tokenImage));
	}
	...
	public void visit(InstrumentDeclaration n) {
		...
		for (int i=0 ; i< this.player.getInstruments().length; i++)
		{
			if(this.player.getInstruments()[i].toString().toUpperCase().contains(n.f2.tokenImage.toUpperCase()))
				{
					this.player.selectInstrument(n.f2.tokenImage);
					break;
				}
		}
	}
	...
	public void visit(Group n) {
		...
		n.f4.accept(this);
		try {
			if(this.inEssay) for(String note : this.currentOthers) this.player.playNote(note, this.currentDuration);
